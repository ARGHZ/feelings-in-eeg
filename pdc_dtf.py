...
def compute_order(X, p_max):
    ...
    for p in range(1, p_max + 1):
        print('computing order: {}'.format(p))
...
def plot_all(freqs, P, name, display_as_grid=True):
    ...
    if display_as_grid:
        ...
        for i in range(N):
            ...
            for j in range(N):
                ...
                plt.figure()
                plt.title(title)
                plt.fill_between(pos_freqs, P[freqs >= 0, i, j], 0)
                plt.xlim([0, np.max(pos_freqs)])
                plt.ylim([0, 1])
                plt.show()


def get_example_from_paper():
